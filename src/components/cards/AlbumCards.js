...
function AlbumCards(props) {
  ...
  const expand = () => {
    console.log(id + 'expand' + expanded)
    setExpanded(true)
  };
  ...
  return album.loading ? (
    ...
    <TruncatedList 
      className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 max-h-72 ${expanded ? "max-h-full" : ""}`}
      alwaysShowTruncator
      renderTruncator={({ hiddenItemsCount }) => {
        if(hiddenItemsCount > 0) {
          return (
            <button className="text-white bg-blue-400 px-3 py-2 rounded" onClick={expand}>
              {hiddenItemsCount} more...
            </button>
          );
        } else {
          return (
            <button className="text-white bg-blue-400 px-3 py-2 rounded" onClick={collapse}>
              hide
            </button>
          );
        }
